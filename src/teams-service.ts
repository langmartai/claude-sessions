...
export class AgentTeamsService {
  ...
  constructor(teamsDir?: string) {
    this.teamsDir = teamsDir || path.join(homedir(), '.claude', 'teams');
  }
...
export function createAgentTeamsService(teamsDir?: string): AgentTeamsService {
  return new AgentTeamsService(teamsDir);
}
